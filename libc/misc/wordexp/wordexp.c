	     ...
	     const char *words, size_t *offset, size_t wordc)
{
  ...
  if (i == 1 + *offset)
      ...
      else
	{
	  ...
	  while ((result = getpwuid_r (uid, &pwd, buffer, buflen, &tpwd)) != 0
		 && errno == ERANGE)
	    {
	      buflen += 1000;
	      buffer = alloca (buflen);
	...
	case ']':
	  if (bracket && paren_depth == 1)
	      ...
	      if (*expr && eval_expr (expr, &numresult) != 0)
		{
	      ...
	      result[20] = '\0';
	      *word = w_addstr (*word, word_length, max_length,
				itoa_word (numresult, &result[20], 10, 0));
	      free (expr);
	      return *word ? 0 : WRDE_NOSPACE;
	    }
...
envsubst:
  ...
  if (env == NULL)
      ...
      if (seen_hash)
	{
	  ...
	  buffer[20] = '\0';
	  value = itoa_word (__libc_argc - 1, &buffer[20], 10, 0);
	  seen_hash = 0;
	}
      else
	{
  ...
  /* Is it a special parameter? */
	...
	{
	  buffer[20] = '\0';
	  value = itoa_word (__getpid (), &buffer[20], 10, 0);
	}
      /* Is it `${#*}' or `${#@}'? */
      else if ((*env == '*' || *env == '@') && seen_hash)
	{
	  buffer[20] = '\0';
	  value = itoa_word (__libc_argc > 0 ? __libc_argc - 1 : 0,
			      &buffer[20], 10, 0);
  ...
  if (seen_hash)
    {
      ...
      param_length[20] = '\0';
      *word = w_addstr (*word, word_length, max_length,
			itoa_word (value ? strlen (value) : 0,
